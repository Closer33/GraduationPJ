...
func makeRoutes() -> Routes {
	...
	routes.add(method: .get, uri: "/echo", handler: {
        ...
        WebSocketHandler(handlerProducer: {
            ...
            echoHandler.socketProtocol = protocols.first
            return echoHandler
...
class EchoHandler: WebSocketSessionHandler {
	...
	func handleSession(request: HTTPRequest, socket: WebSocket) {
        ...
        if isFirstEstablished {
            WebSocketManager.sharedInstance.addSocket(socket: socket, key: socketProtocol!)
		...
		socket.readStringMessage {
            ...
            WebSocketManager.sharedInstance.getAnotherSocket(socket: socket, key: self.socketProtocol!)?.sendStringMessage(string: string, final: true, completion: {
